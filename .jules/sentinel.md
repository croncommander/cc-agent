...
## 2026-01-01 - Unbounded Socket Connection DoS
Risk: The daemon accepted an unlimited number of concurrent connections (`net.Listener.Accept` loop), spawning a goroutine for each. A local attacker could flood the socket with thousands of connections, causing resource exhaustion (goroutines, memory) and crashing the daemon.
Learning: `net.Listener` does not inherently limit the number of active connections. An explicit semaphore (buffered channel) is required to apply backpressure and limit concurrent handlers. Relying on OS limits is insufficient as goroutines are cheap but not free.
Action: Implemented a semaphore-based concurrency limit (default 50) in `startSocketListener`. Connections beyond the limit block in the `Accept` loop (and thus the OS backlog) until a slot is free, effectively throttling the load.
