...
## 2026-01-29 - Unbounded Socket Concurrency DoS
**Risk:** The daemon's socket listener (`Accept` loop) spawned a new goroutine for every incoming connection without limit. A local attacker could rapidly open thousands of connections, causing the daemon to spawn thousands of goroutines, exhausting system memory and CPU (scheduler thrashing), leading to a crash or system instability.
**Learning:** `net.Listen` and `Accept` do not inherently limit concurrency. In Go, spawning a goroutine is cheap but not free. Public-facing (or multi-user) listeners must strictly limit concurrent active connections using a semaphore pattern (buffered channel) to apply backpressure to the OS socket backlog.
**Action:** Implemented a semaphore with a limit of 50 concurrent connections. When the limit is reached, the `Accept` loop blocks, forcing new connections to wait in the OS backlog until a slot opens up.
