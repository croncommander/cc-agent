...
## 2026-02-06 - Missing Peer Verification on Unix Socket
**Risk:** `cc-agent exec` blindly connected to Unix sockets, allowing local attackers to intercept sensitive execution reports (stdout/stderr) by pre-creating sockets in `/tmp`.
**Learning:** Do not assume "Internal" IPC is secure just because it uses Unix sockets. If the socket path is in a shared directory (like `/tmp`), peer verification (`SO_PEERCRED`) is mandatory to prevent spoofing.
**Action:** Implemented strict `SO_PEERCRED` verification in `cmd/exec.go` to ensure the peer is the current user or root.
