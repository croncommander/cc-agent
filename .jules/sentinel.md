...
## 2026-01-17 - Socket Concurrency Exhaustion (DoS)
Risk: The daemon's socket listener spawned a new goroutine for every incoming connection without limit. A malicious local user could flood the socket, causing the daemon to spawn thousands of goroutines, leading to memory exhaustion (OOM) or scheduler overload.
Learning: `net.Listener.Accept()` loops must always have a mechanism to limit concurrency (semaphore or worker pool), especially when handling connections from untrusted or semi-trusted sources. Relying solely on per-connection timeouts is insufficient against rapid connection flooding.
Action: Implemented a semaphore (buffered channel) to strictly limit concurrent connections to 50. Incoming connections beyond this limit cause the main loop to block, creating backpressure on the OS socket backlog.
