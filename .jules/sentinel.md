...
## 2026-01-06 - Unbounded Socket Connection DoS
Risk: The daemon accepted unlimited concurrent socket connections, spawning a new goroutine for each. A local attacker could trigger a Denial of Service (DoS) by opening thousands of connections, exhausting system resources (goroutines, memory) and causing the daemon to crash or become unresponsive.
Learning: `Accept()` loops in Go must have backpressure or limits. While Go handles goroutines efficiently, they are not free. A simple buffered channel semaphore is an effective pattern to enforce a hard limit on concurrency without complex connection pooling.
Action: Implemented a semaphore (buffered channel) to strictly limit concurrent socket connections to 50. Excess connections will block in the kernel backlog until a slot is free, preserving daemon stability.
