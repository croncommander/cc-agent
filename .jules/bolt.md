...
## 2026-01-25 - [System Call Hoisting] **Learning:** `os.Executable()` on Linux involves a `readlink` syscall and is not cached by the Go runtime. Calling it inside a loop (e.g. 1000 times) caused huge overhead (3000 allocs vs 5). **Action:** Always hoist `os.Executable()` and similar syscall wrappers out of loops.
