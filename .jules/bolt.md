...
## 2026-01-14 - [Syscall Hoisting] **Learning:** `os.Executable()` performs a `readlink` syscall on Linux and is not cached. Calling it inside a loop (e.g., for each job) causes massive performance degradation. **Action:** Always hoist system calls like `os.Executable()` or `os.Getwd()` out of hot loops.
