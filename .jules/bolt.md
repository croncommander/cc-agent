...
## 2026-01-21 - [Syscall Hoisting] **Learning:** `os.Executable()` on Linux performs a `readlink` syscall which is not cached by the runtime. Calling it inside a loop (e.g., cron generation) causes significant overhead. **Action:** Always hoist `os.Executable()` and similar system calls (like `os.Getwd()`) outside of hot loops.
