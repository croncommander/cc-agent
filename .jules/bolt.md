...
## 2025-01-26 - [Syscall Hoisting] **Learning:** `os.Executable()` on Linux performs a `readlink` syscall and allocates new string on every call. It is not cached. Calling it in a loop (e.g., 1000 times) is ~8x slower than hoisting it. **Action:** Always hoist system calls like `os.Executable()` or `os.Getwd()` out of hot loops.
