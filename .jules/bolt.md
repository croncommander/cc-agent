...
## 2026-01-24 - [Syscall Hoisting] **Learning:** `os.Executable()` on Linux performs a `readlink` syscall and is not cached. Inside a loop, this causes significant overhead (9x slowdown). **Action:** Always hoist `os.Executable()` (and similar syscall-based lookups like `os.Getwd()`) out of loops.
