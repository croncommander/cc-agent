...
var (
	...
	// maxConcurrentConnections limits the number of active socket handlers to prevent resource exhaustion (DoS).
...
func (d *daemon) startSocketListener() {
	...
	for {
		...
		// This applies backpressure: if we are at capacity, we stop accepting
		// new connections (effectively, as we block here holding one unserved connection).
		sem <- struct{}{}
